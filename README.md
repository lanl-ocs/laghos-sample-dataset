...
Each parquet file consists of a single row group and 10 columns: `element_id`, `vertex_id`, `v_x`, `v_y`, `v_z`, `rho`, `e`, `x`, `y`, and `z` (see schema information below). Each column represents an attribute of a nodal value, and is formatted as an array of snappy-compressed, plain-encoded parquet pages. There are 16-32 pages per column per file depending on data types. Applying snappy compression on all columns reduces the size of each parquet file from 288MB to 97MB. Before uploading to github, these parquet files are additionally compressed using brotli. This further reduces the parquet file size from 97MB to 39MB so that github does not complain file being too large.
